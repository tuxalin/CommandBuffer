...
struct NvUnixScopeLock
{
	NvUnixScopeLock(pthread_mutex_t& m) : mMutex(m)
	{
...
void SyncImpl::reset()
{
	NvUnixScopeLock lock(getSync(this)->mutex);
	getSync(this)->is_set = false;
}
...
bool SyncImpl::wait(uint32_t ms)
{
	...
	if(!getSync(this)->is_set)
	{
		if(ms == uint32_t(-1))
		{
			...
			NV_ASSERT((!status && getSync(this)->is_set) || (lastSetCounter != getSync(this)->setCounter));
		}
		else
		{
			timespec ts;
			...
			NV_ASSERT((!status && getSync(this)->is_set) || (status == ETIMEDOUT) ||
