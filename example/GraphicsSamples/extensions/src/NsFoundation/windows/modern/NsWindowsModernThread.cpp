...
volatile DWORD gTlsSlots[NV_TLS_MAX_SLOTS] = { 0 };

// Actual TLS storage, unique per thread
__declspec(thread) void* tlsData[NV_TLS_MAX_SLOTS];

// Sets all the slot values to zero.  This method will be called every time a new Nv thread is created
void NvClearTlsData()
{
	for(unsigned int i = 0; i < NV_TLS_MAX_SLOTS; i++)
	{
		tlsData[i] = NULL;
	}
}
...
class _ThreadImpl
{
  public:
	...
	NV_FORCE_INLINE _ThreadImpl(ThreadImpl::ExecuteFn fn_, void* arg_);
...
NV_FORCE_INLINE _ThreadImpl::_ThreadImpl(ThreadImpl::ExecuteFn fn_, void* arg_)
...
{
	NV_PLACEMENT_NEW(getThread(this), _ThreadImpl)(NULL, NULL);
}

ThreadImpl::ThreadImpl(ExecuteFn fn, void* arg)
{
	NV_PLACEMENT_NEW(getThread(this), _ThreadImpl)(fn, arg);
	start(0, NULL);
}
...
void ThreadImpl::start(uint32_t /*stackSize*/, Runnable* runnable)
{
	...
	{
		NvThreadStart((LPVOID) this); // function to run async
	};
...
uint32_t TlsAlloc()
{
	for(unsigned int i = 0; i < NV_TLS_MAX_SLOTS; i++)
		if(InterlockedCompareExchange(&gTlsSlots[i], TRUE, FALSE) == FALSE)
			return i;
	NV_ASSERT(!"Cannot find a free tls slot, increase NV_TLS_MAX_SLOTS");
	return (uint32_t) - 1;
}

void TlsFree(uint32_t index)
{
	NV_ASSERT(index < NV_TLS_MAX_SLOTS);
