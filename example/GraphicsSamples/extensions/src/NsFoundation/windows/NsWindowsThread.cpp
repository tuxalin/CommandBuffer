...
uint32_t ThreadImpl::getNbPhysicalCores()
{
	if(!gPhysicalCoreCount)
	{
		...
		{
			buffer = (PSYSTEM_LOGICAL_PROCESSOR_INFORMATION)NvAlloca(returnLength);
		}
		else
		{
			getErrorCallback().reportError(NvErrorCode::eINTERNAL_ERROR,
			                               "Error querying buffer size for number of physical processors", __FILE__,
			                               __LINE__);
			return 0;
		}
		...
		if(rc != TRUE)
		{
			getErrorCallback().reportError(NvErrorCode::eINTERNAL_ERROR, "Error querying number of physical processors",
			                               __FILE__, __LINE__);
			return 0;
		}
...
{
	DWORD rv = ::TlsAlloc();
	NV_ASSERT(rv != TLS_OUT_OF_INDEXES);
	return (uint32_t)rv;
}
