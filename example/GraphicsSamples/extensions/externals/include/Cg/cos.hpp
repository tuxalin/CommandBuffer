...
static inline __CGvector<typename __CGtype_trait<T>::realType,N> cos(const __CGvector<T,N> & v)
{
    __CGvector<T,N> rv;
    for (int i=0; i<N; i++)
        rv[i] = ::cos(v[i]);
...
static inline __CGvector<typename __CGtype_trait<T>::realType,N> cos(const __CGvector_usage<T,N,Tstore> & v)
{
    __CGvector<T,N> rv;
    for (int i=0; i<N; i++)
        rv[i] = ::cos(v[i]);
