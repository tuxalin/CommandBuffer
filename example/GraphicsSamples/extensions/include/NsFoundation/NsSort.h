...
#ifndef NV_NSFOUNDATION_NSSORT_H
#define NV_NSFOUNDATION_NSSORT_H
...
#define NV_SORT_PARANOIA NV_DEBUG
          ...
          const uint32_t initialStackSize = 32)
{
	...
	NV_ALLOCA(stackMem, int32_t, initialStackSize);
	...
	if(last > first)
	{
		for(;;)
		{
			while(last > first)
			{
				NV_ASSERT(first >= 0 && last < (int32_t)count);
	...
	for(uint32_t i = 1; i < count; i++)
		NV_ASSERT(!compare(elements[i], elements[i - 1]));
#endif
}
...
#endif // #ifndef NV_NSFOUNDATION_NSSORT_H
