  ...
  private:
	...
	void reserveInternal(uint32_t size)
	{
		...
		Entry* newEntries;
		{
			...
			newBuffer = (uint8_t*)Allocator::allocate(newBufferByteSize, __FILE__, __LINE__);
			NV_ASSERT(newBuffer);

			newHash = (uint32_t*)(newBuffer + newHashByteOffset);
			newEntriesNext = (uint32_t*)(newBuffer + newEntriesNextBytesOffset);
