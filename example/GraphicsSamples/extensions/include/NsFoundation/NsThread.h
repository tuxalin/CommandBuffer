...
#ifndef NV_NSFOUNDATION_NSTHREAD_H
#define NV_NSFOUNDATION_NSTHREAD_H
...
#if NV_WINDOWS_FAMILY || NV_WINRT || NV_XBOXONE
#define NvSpinLockPause() __asm pause
#if NV_WINRT
#define NV_TLS_MAX_SLOTS 64
#endif
#elif NV_X360
#define NvSpinLockPause() __asm nop
#elif NV_LINUX || NV_ANDROID || NV_PS4 || NV_APPLE_FAMILY
...
class NV_FOUNDATION_API ThreadImpl
{
  public:
  ...
  public:
	...
	*/
	ThreadT(const Alloc& alloc = Alloc()) : Alloc(alloc)
	{
		...
		NV_PLACEMENT_NEW(mImpl, ThreadImpl)();
	}
	...
	*/
	...
	{
		mImpl = reinterpret_cast<ThreadImpl*>(Alloc::allocate(ThreadImpl::getSize(), __FILE__, __LINE__));
		NV_PLACEMENT_NEW(mImpl, ThreadImpl)(fn, arg);
	}
...
NV_FOUNDATION_API uint32_t TlsAlloc();
NV_FOUNDATION_API void TlsFree(uint32_t index);
NV_FOUNDATION_API void* TlsGet(uint32_t index);
NV_FOUNDATION_API uint32_t TlsSet(uint32_t index, void* value);

} // namespace shdfnd
} // namespace nvidia

#endif // #ifndef NV_NSFOUNDATION_NSTHREAD_H
