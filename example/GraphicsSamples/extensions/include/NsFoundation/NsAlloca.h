...
#ifndef NV_NSFOUNDATION_NSALLOCA_H
#define NV_NSFOUNDATION_NSALLOCA_H
...
#ifdef __SPU__
#define NV_ALLOCA(var, type, count) type* var = (type*)NvAlloca(sizeof(type) * (count))
#else
#define NV_ALLOCA(var, type, count)                                                                                    \
	...
	{                                                                                                                  \
		...
		if(var.mOwned)                                                                                                 \
			var.mPointer = (type*)nvidia::shdfnd::TempAllocator().allocate(size, __FILE__, __LINE__);                  \
		else                                                                                                           \
			var.mPointer = (type*)NvAlloca(size);                                                                      \
	}
#endif
#endif // #ifndef NV_NSFOUNDATION_NSALLOCA_H
