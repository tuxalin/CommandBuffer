...
#ifndef NV_NSFOUNDATION_NSSORTINTERNALS_H
#define NV_NSFOUNDATION_NSSORTINTERNALS_H
...
NV_INLINE void median3(T* elements, int32_t first, int32_t last, Predicate& compare)
{
	/*
...
template <class T, class Predicate>
NV_INLINE int32_t partition(T* elements, int32_t first, int32_t last, Predicate& compare)
{
	...
	*/
	...
	for(;;)
	{
		...
		NV_ASSERT(i <= last && j >= first);
		swap(elements[i], elements[j]);
	}
	// put the pivot in place

	NV_ASSERT(i <= last && first <= (last - 1));
...
template <class T, class Predicate>
NV_INLINE void smallSort(T* elements, int32_t first, int32_t last, Predicate& compare)
{
...
class Stack
{
  ...
  public:
	...
	{
		mCapacity *= 2;
		int32_t* newMem = (int32_t*)mAllocator.allocate(sizeof(int32_t) * mCapacity, __FILE__, __LINE__);
	...
	NV_INLINE void push(int32_t start, int32_t end)
	{
	...
	NV_INLINE void pop(int32_t& start, int32_t& end)
	{
		NV_ASSERT(!empty());
		end = mMemory[--mSize];
		start = mMemory[--mSize];
...
#endif // #ifndef NV_NSFOUNDATION_NSSORTINTERNALS_H
